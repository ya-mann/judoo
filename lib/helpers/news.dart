...
class News {
  ...
  static Future<List<News>> get({numberOfNews: 1}) async {
    final response =
        await http.get(Uri.parse('https://www.ffjudo.com/actualites/'));
    List<News> news = [];
    if (response.statusCode == 200) {
      final document = parse(utf8.decode(response.bodyBytes));
      final newsBlock = document.querySelectorAll('.listing_bloc').getRange(0, numberOfNews);
      newsBlock.forEach((element) {
        ...
        news.add(News(
          ...
          thumbnailUrl: element.getElementsByTagName('img').first.attributes['src'],
